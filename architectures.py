...
def round_robin(agents, judge, topic, num_rounds, config):
    ...
    for round_num in range(1, num_rounds + 1):
        ...
        for agent in agents:
            response, tokens = agent.respond(history, config)
            debate_log.append({"agent": agent.name, "round": round_num, "text": response, "tokens": tokens})
            history.append(f"{agent.name}: {response}")
            print(f"\n[{agent.name} ({tokens} tok.)]: {response}")
